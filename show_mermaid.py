...
def initialize_app():
    """
    Initialize the Streamlit application with basic settings and styles.
    """
...
with col_text:
    st.session_state.text_input = st.text_area(
        ...
        help="Paste your document text here.",
        placeholder="Enter your document text..."
    )

# Check and set prompt_for_diagram_creation in session state
...
with st.expander('3. Edit prompt for a diagram creation'):
    st.session_state.selected_prompt_for_diagram_creation = st.text_area(
        "",
        height=300,
        value=st.session_state.prompt_for_diagram_creation.get(graph_type)
    )
...
if make_summary:
    with st.expander('**Settings for summarization 💬**'):
        ...
        st.session_state.selected_prompt_for_summarization = st.text_area(
            "Prompt for summarization",
            height=300,
            value=st.session_state.selected_prompt_for_summarization
        )
...
if st.button('Generate Mermaid!') and (st.session_state.settings_current != st.session_state.settings_previous):
    ...
    if make_summary:
        st.session_state.mermaid_input = get_llm_response_model_specific(
            diagram_model,
            st.session_state.selected_prompt_for_summarization,
    ...
    (st.session_state.mermaid_code,
     ...
     st.session_state.text_to_download) = get_mermaid_data(
...
# Chat column
with col_chat:
    st.session_state.question_current = st.text_area(
        label="**Chat on a diagram (Agent already knows about your data/code)**",
        height=130,
        placeholder="Fix the diagram to add a new block - 'Current treatment options'"
    )
    ...
    if st.session_state.question_current and run_chat:
        with get_openai_callback() as cb:
            if not st.session_state.internal_history:
                ...
                (st.session_state.mermaid_code_chat_based,
                 st.session_state.mermaid_link_chat_based,
                 st.session_state.text_to_download_chat_based) = get_mermaid_data(
                    st.session_state.question_current,
                    assistant_prompt,
                    "gpt-4o",
                    0.2,
